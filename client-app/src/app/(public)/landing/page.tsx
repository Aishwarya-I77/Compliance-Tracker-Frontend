...
const Navbar = styled.nav<{ $animate: boolean }>`
...
const Logo = styled.img`
  width: 161.78px; height: 37px; object-fit: contain;
`;

const NavButtons = styled.div`display: flex; align-items: center; gap: 10px;`;

const BaseBtn = styled.button`
  width: 100px; height: 43px; padding: 12px 20px;
...
const LoginBtn = styled(BaseBtn)`
  ...
  &:hover { border-color: #314f8b; color: #314f8b; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(49,79,139,0.15); }
  &:active { background: #1e293b; color: #fff; border-color: #1e293b; transform: translateY(0); }
`;

const SignupBtn = styled(BaseBtn)`
  color: #fff; background: #314f8b; font-weight: 400;
  &:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(49,79,139,0.35); }
  &:active { background: #1e293b; opacity: 1; transform: translateY(0); }
`;
...
const Heading = styled.h1<{ $animate: boolean }>`
  font-size: 64px; font-weight: 600; color: #1e293b;
  line-height: 1.15; margin: 0; letter-spacing: -1px;
  max-width: 980px;
  opacity: ${({ $animate }) => ($animate ? "0" : "1")};
  ${({ $animate }) => $animate && css`
    animation: ${slideLeft} 0.8s cubic-bezier(0.25,0.46,0.45,0.94) 0.2s forwards;
  `}
  @media (max-width: 768px) { font-size: 36px; }
`;

const Subtext = styled.p<{ $animate: boolean }>`
  font-size: 20px; font-weight: 400; color: #475569;
  line-height: 1.6; margin: 36px 0 0; max-width: 820px;
  opacity: ${({ $animate }) => ($animate ? "0" : "1")};
  ${({ $animate }) => $animate && css`
    animation: ${slideRight} 0.8s cubic-bezier(0.25,0.46,0.45,0.94) 0.45s forwards;
  `}
  @media (max-width: 768px) { font-size: 16px; }
`;

const BtnText = styled.span`transition: transform 0.2s ease;`;
const BtnArrow = styled.span`
  opacity: 0; transform: translateX(-8px);
  transition: opacity 0.2s ease, transform 0.2s ease; font-size: 18px;
`;

const GetStartedBtn = styled.button<{ $animate: boolean }>`
  width: 230px; height: 56px; margin-top: 36px;
  font-family: inherit; font-size: 20px; font-weight: 500;
  color: #fff; background: #314f8b;
  ...
  &:hover {
    ...
    ${BtnArrow} { opacity: 1; transform: translateX(0); }
    ${BtnText} { transform: translateX(-4px); }
  }
  &:active { background: #1e293b; transform: translateY(0); }
  @media (max-width: 768px) { width: 180px; height: 48px; font-size: 16px; }
`;
